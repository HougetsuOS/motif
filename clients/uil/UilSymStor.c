...
sym_dump_symbol (sym_entry_type *az_symbol_entry)
{
	...
	default:
		...
		_debug_output("%lx  unknown type: %d  size: %d  byte: %x\n",
...
sym_dump_widget( XmConst sym_widget_entry_type *az_widget_entry )
{
    ...
    _debug_output (
	"  %s %s  controls: %lx  callbacks: %lx  arguments: %lx  parent_list: %lx\n", 
...
sym_dump_argument( XmConst sym_argument_entry_type *az_argument_entry )
{
    ...
    _debug_output ( "  arg name: %lx  arg value: %lx\n", 
	(unsigned long)az_argument_entry->az_arg_name,
	(unsigned long)az_argument_entry->az_arg_value );
...
/*    These are for control objects only.     */
    ...
    _debug_output ( "  obj: %lx\n", 
	(unsigned long)az_control_entry->az_con_obj );
...
sym_dump_callback( XmConst sym_callback_entry_type *az_callback_entry )
{
    ...
    _debug_output ( "  reason name: %lx  proc ref: %lx  proc ref list: %lx\n", 
	(unsigned long)az_callback_entry->az_call_reason_name,
...
sym_dump_name( XmConst sym_name_entry_type *az_name_entry )
{
    ...
    _debug_output
	( "%lx name size: %d  next name: %lx  object: %lx",
...
sym_dump_module( XmConst sym_module_entry_type *az_module_entry )
{
    ...
    _debug_output
	( "%lx module size: %d  name: %lx  version: %lx \n",
	  (unsigned long)az_module_entry,
	  az_module_entry->header.w_node_size,
	  (unsigned long)az_module_entry->obj_header.az_name,
...
sym_dump_color_item( XmConst sym_color_item_entry_type *az_color_item_entry )
{
    ...
    _debug_output
	( "%lx color_item  size: %d  letter: %c  index: %d  color: %lx  next: %lx\n",
...
{
    _debug_output
	( "%lx parent_list  size: %d  parent: %lx  next: %lx \n",
		    ...
		    XmConst sym_external_def_entry_type *az_external_def_entry)
{
    ...
    _debug_output
	( "%lx external def  size: %d  next external: %lx  object: %lx \n",
...
sym_dump_proc_def( XmConst sym_proc_def_entry_type *az_proc_def_entry )
{
    ...
    _debug_output
	( "%lx proc def  size: %d  name: %lx %s%s%s%s  count: %d  %s\n",
...
sym_dump_proc_ref( XmConst sym_proc_ref_entry_type *az_proc_ref_entry )
{
    ...
    _debug_output
	( "%lx proc ref  size: %d  proc def: %lx  value: %lx\n",
...
sym_dump_forward_ref(XmConst sym_forward_ref_entry_type *az_forward_ref_entry)
{
    ...
    _debug_output
	( "%lx forward ref  size: %d  next ref: %lx  location: %lx  %s  parent: %lx\n",
...
sym_dump_value( XmConst sym_value_entry_type *az_value_entry )
{
    ...
    _debug_output
	( "%lx value  size: %d  name: %lx  %s%s%s%s",
...
common_special_type:
	...
	if ( (az_value_entry->b_aux_flags & sym_m_table_entry) != 0 ) {
	    _debug_output("  next table entry: %lx",
	...
	if (az_value_entry->b_charset != sym_k_userdefined_charset) 
	    ...
	    _debug_output("  font  charset: userdefined(%lx)",
			  (unsigned long)diag_charset_text( (long)az_value_entry->az_charset_value ) );
	...
	if (az_value_entry->b_charset != sym_k_userdefined_charset) 
		...
		case XmSTRING_DIRECTION_L_TO_R:
		    _debug_output
			("  string length: %d\n  charset: userdefined(%lx)  L_TO_R", 
			  az_value_entry->w_length,
			  (unsigned long)az_value_entry->az_charset_value);
		    break;
		case XmSTRING_DIRECTION_R_TO_L:
		    _debug_output
			("  string length: %d\n  charset: userdefined(%lx)  R_TO_L", 
			  az_value_entry->w_length,
			  (unsigned long)az_value_entry->az_charset_value);
		    break;
		}
...
check_for_table_value:
	...
	_debug_output("  icon  width: %d  height: %d  colors: %lx  rows: %lx \n", 
...
common_table:
	...
	_debug_output("  %s  first table entry: %lx\n",
        ...
        for (index = 0;  index < (int)az_value_entry->b_table_count;  index++)
	{
	    ...
	    _debug_output( "    letter: %c  index: %d  color: %lx\n",
...
sym_dump_obj_header (XmConst sym_obj_entry_type *az_obj_entry)
{
	...
	_debug_output ("  name: %lx", (unsigned long)az_obj_entry->obj_header.az_name);
    }

    if (az_obj_entry->obj_header.az_reference != NULL) {
	_debug_output ("  reference: %lx",
			(unsigned long)az_obj_entry->obj_header.az_reference);
    }

    if (az_obj_entry->obj_header.az_next != NULL) {
	_debug_output ("  next: %lx", (unsigned long)az_obj_entry->obj_header.az_next);
    }
...
sym_dump_include_file ( sym_include_file_entry_type *az_symbol_entry )
{
    ...
    _debug_output ("%lx  include file  file name: %s  full file name: %s\n",
	(unsigned long)az_symbol_entry,
...
sym_dump_section ( sym_section_entry_type *az_symbol_entry )
{
    _debug_output ("%lx  %s section  prev section : %lx  next section: %lx  entries: %lx\n",
	(unsigned long)az_symbol_entry,
	sym_section_text(az_symbol_entry->header.b_type),
	(unsigned long)az_symbol_entry->prev_section, (unsigned long)az_symbol_entry->next, 
...
sym_dump_object_variant ( sym_def_obj_entry_type * az_symbol_entry )
{
    _debug_output ("%lx  default obj var  next: %lx  object info: %d, variant_info: %d\n",
	(unsigned long)az_symbol_entry,
...
sym_dump_root_entry ( sym_root_entry_type *az_symbol_entry )
{
    _debug_output (  "%lx  root  tag: %d  module: %lx  sections: %lx\n  module tail: ",
	(unsigned long)az_symbol_entry,
	az_symbol_entry->header.b_tag,
