...
ScaleCB(Widget w, XtPointer client, XtPointer call)
{
	...
	XtVaSetValues(G_tree, resources[(unsigned long)(XtPointer) client], cbs->value, NULL);
}
...
void MakeControlPanel(Widget right_pane)
{
	...
	*/
	...
	for (i=0;i<XtNumber(resources);i++)
		...
		XtAddCallback(scale, XmNvalueChangedCallback, ScaleCB, (XtPointer)(unsigned long) i);
		XtManageChild(scale);
		}
	}
